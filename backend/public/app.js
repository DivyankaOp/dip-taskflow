...
// ── Upload view ───────────────────────────────────────────────────────────────
function loadMonthlyReportUploadView() {
  ...
  // Populate project dropdown from master data
  fillSelect(document.getElementById('mr-project'), state.master.projects, { placeholder: 'Select Project' });
 
  // Wire up the drop zone (only once)
  const dropzone  = document.getElementById('mrDropzone');
  const fileInput = document.getElementById('mrFolderInput');
  if (dropzone && !dropzone._mrReady) {
    dropzone._mrReady = true;
    ...
    dropzone.addEventListener('drop', (e) => {
      ...
      mrTraverseDropItems(e.dataTransfer.items);
    });
 
    fileInput.addEventListener('change', () => {
      mrUpdateFileCount(Array.from(fileInput.files));
    });
 
    document.getElementById('mrResetBtn').addEventListener('click', mrResetUploadForm);
  }
...
function mrResetUploadForm() {
  window._mrFiles = null;
  const form     = document.getElementById('monthlyReportForm');
  const countEl  = document.getElementById('mrFileCount');
  const progWrap = document.getElementById('mrProgressWrap');
  const msgEl    = document.getElementById('mrFormMsg');
  const inp      = document.getElementById('mrFolderInput');
  if (form)     form.reset();
...
// Traverse drag-and-drop file entries (handles nested folders)
function mrTraverseDropItems(dataTransferItems) {
  const collected = [];
  let   pending   = 0;
 
  function check() {
    if (pending === 0) {
      window._mrFiles = collected;
      mrUpdateFileCount(collected);
    }
  }
  ...
  function traverse(entry) {
    if (entry.isFile) {
      pending++;
      entry.file((file) => { collected.push(file); pending--; check(); });
      ...
      function readChunk() {
        reader.readEntries((entries) => {
          ...
          readChunk(); // readEntries may return only 100 at a time
        });
      }
      readChunk();
    }
  ...
  if (pending === 0) check(); // empty drop
}
 
function mrUpdateFileCount(filesArr) {
  const countEl  = document.getElementById('mrFileCount');
  const totalSize = filesArr.reduce((s, f) => s + f.size, 0);
  const sizeMB    = (totalSize / (1024 * 1024)).toFixed(1);
  countEl.textContent = `${filesArr.length} file(s) selected — ${sizeMB} MB`;
  countEl.hidden = false;
  window._mrFiles = filesArr;
}
...
document.getElementById('monthlyReportForm').addEventListener('submit', async function (e) {
  ...
  const msgEl     = document.getElementById('mrFormMsg');
  const progWrap  = document.getElementById('mrProgressWrap');
  const progFill  = document.getElementById('mrProgressFill');
  const progPct   = document.getElementById('mrProgressPct');
  const submitBtn = document.getElementById('mrSubmitBtn');
 
  msgEl.hidden = true;
 
  const files = window._mrFiles || Array.from(document.getElementById('mrFolderInput').files);
  if (!files.length) {
    msgEl.textContent = 'Please select a folder to upload';
    msgEl.hidden = false;
    return;
  }
  ...
  const tick = setInterval(() => {
    ...
    progPct.textContent  = `${fakeProgress}%`;
  }, 300);
 
  try {
    await api('/monthly-reports', { method: 'POST', body: formData, isForm: true });
 
    clearInterval(tick);
    progFill.style.width = '100%';
    progPct.textContent  = '100%';
    showToast('Monthly report uploaded ✅', 'success');
 
    setTimeout(() => {
      mrResetUploadForm();
      // Re-populate year & project after reset
      loadMonthlyReportUploadView();
    }, 1200);
...
// ── All Reports view ──────────────────────────────────────────────────────────
async function loadAllMonthlyReports() {
  const tbody = document.getElementById('monthlyReportsTableBody');
  if (!tbody) return;
 
  // Populate project filter from master data
  fillSelect(document.getElementById('mrFilterProject'), state.master.projects, { placeholder: 'All Projects' });
 
  // Populate year filter
  const yearSel = document.getElementById('mrFilterYear');
  ...
  // Apply-filter button
  const applyBtn = document.getElementById('mrApplyFilter');
  if (applyBtn && !applyBtn._mrReady) {
    applyBtn._mrReady = true;
    applyBtn.addEventListener('click', loadAllMonthlyReports);
  }
  ...
  try {
    const params = new URLSearchParams();
    const fp = document.getElementById('mrFilterProject').value;
    const fm = document.getElementById('mrFilterMonth').value;
    const fy = document.getElementById('mrFilterYear').value;
    if (fp) params.set('project_id', fp);
    if (fm) params.set('month',      fm);
    if (fy) params.set('year',       fy);
 
    const reports = await api(`/monthly-reports${params.toString() ? '?' + params : ''}`);
    ...
    tbody.innerHTML = reports.map((r, i) => {
      const bytes   = r.total_size_bytes || 0;
      ...
      return `<tr>
        <td>${i + 1}</td>
        <td><strong>${MONTHS_FULL[r.month] || r.month}</strong></td>
        <td>${r.year}</td>
        <td>${escapeHtml(r.project?.name ?? '—')}</td>
        <td>${r.total_files} file(s)</td>
        <td>${sizeStr}</td>
        <td>${escapeHtml(r.uploaded_by_user?.full_name ?? '—')}</td>
        <td>${fmtDateOnly(r.created_at)}</td>
        <td>
          <button class="ghost-btn-text"
            onclick="mrViewFiles(${JSON.stringify(r.file_urls || []).replace(/"/g, '&quot;')})">
            👁 View Files
          </button>
          <button class="ghost-btn-text" style="color:var(--rose)"
            onclick="mrDeleteReport('${r.id}')">
            🗑 Delete
          </button>
        </td>
      </tr>`;
    ...
    tbody.innerHTML = `<tr><td colspan="9" class="empty-state">Failed: ${escapeHtml(err.message)}</td></tr>`;
  }
}
 
// Opens a new tab listing all files in the report as clickable links
function mrViewFiles(fileUrls) {
  if (!fileUrls || !fileUrls.length) { showToast('No files in this report', 'error'); return; }
  const win = window.open('', '_blank');
  const rows = fileUrls.map((url) => {
    const name = decodeURIComponent(url.split('/').pop().replace(/^\d+_/, ''));
    return `<li><a href="${url}" target="_blank" rel="noopener">📄 ${name}</a></li>`;
  }).join('');
  win.document.write(`
    <!DOCTYPE html><html><head><title>Report Files</title>
    <style>
      body { font-family: Arial, sans-serif; padding: 28px; max-width: 800px; margin: auto; }
      h2   { margin-bottom: 16px; }
      ul   { list-style: none; padding: 0; }
      li   { margin: 10px 0; border-bottom: 1px solid #eee; padding-bottom: 10px; }
      a    { color: #2563eb; text-decoration: none; font-size: 14px; }
      a:hover { text-decoration: underline; }
    </style></head>
    <body>
      <h2>📁 Report Files (${fileUrls.length})</h2>
      <ul>${rows}</ul>
    </body></html>
  `);
  win.document.close();
...
async function mrDeleteReport(id) {
  if (!confirm('Delete this monthly report and all its uploaded files? This cannot be undone.')) return;
  try {
    await api(`/monthly-reports/${id}`, { method: 'DELETE' });
